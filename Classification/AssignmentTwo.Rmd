...
Order of categorical values look weired. So, they will be reordered.
...
There is an high imbalance in original marketData. So, Mix of Oversampling and Undersampling is used to rescale the data in order to avoid bias.

```{r}
library(ROSE)
newData <- ovun.sample(Subscribed~.,data=marketData,method="both")$data
...
Simple Decision Tree is made to observe which variables are important. It's classification accuracy in both test set and validation set is observed to be quite low.
...
So, based on the result, Contact, Default and Job are three most important features for Personal+Financial. It is also matched with our initial intuition. Prediction accuracy is around 0.85 in both training set and validation set.
...
According to this model, Duration, PrevDays and Month are three most important features. Prediction accuracy is improved a lot closing to 0.99 in training set and 0.91 in validation set.

It is observed that people are more likely to subscribe if they are engaged before campaign and engaged again during campaign period since Duration, PreDays, Month are all related to campaign.The longer the engagement duration, the more likely people will subscribe the service.
